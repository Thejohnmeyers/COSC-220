...
using namespace std;
//use in order traverse for the print out the tree 
...
{   
	...
	while(t != NULL && !(item == t->nodeValue))
		if (item < t->nodeValue)
			t = t->left;	//add one line code here 
		else 
			t = t->right;//add one line code here 
...
pair<typename stree<T>::iterator, bool> stree<T>::insert(const T& item)
{
	...
	if (parent == NULL)
		...
		parent->right = newNode;	//add one line code here 
...
template <typename T>
void stree<T>::displayTree(stnode<T> *t)
{
	    stack<stnode<T> *> s;
    stnode<T> *curr = root;
